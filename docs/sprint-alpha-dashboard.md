...
- C1 now has source-only runtime, observer-test, exhaustive verifier-case/fault,
  closure-acceptance, and v1 helper-evidence contracts. They remain unwired and
  non-activating. C2/C3V must still provide the reviewed controller, fixtures,
  candidate closure, tool pins, retained runtime evidence, and exact-main proof;
  C3A must separately accept that evidence before any helper build or test.
