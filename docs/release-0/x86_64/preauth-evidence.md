...
Prompt 7A uses a two-phase evidence model. Committed `phase=prepared` records bind immutable inputs and deliberately contain no Git commit, workflow run, or generated archive claim; this avoids an impossible commit self-reference. Each push and pull-request route checks out the explicit branch head, rejects merge-ref or event mismatch, independently builds the closure, and emits a `phase=attested` record bound to that checked-out SHA, event and run ID. The attestation gives separate typed fields to the canonical archive, Buildx Docker-config descriptor, Docker config, byte-present canonical OCI index, selected OCI manifest, ordered layer descriptor set, ordered rootfs diff-ID set and loaded image config, then binds packages, profile, artifact, disk and closure. The pinned Docker-driver transformation requires Buildx descriptor = Docker config = loaded image config, while the index, selected OCI manifest and archive remain different identities. Stale phase, head, run, archive, typed-image edge, or metadata-only evidence is rejected. Run URLs and final exact-head hashes are durable PR evidence, not committed authorization.
