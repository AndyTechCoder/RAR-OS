...
Executable accepted routes run only in the official Rust 1.95.0 OCI image pinned by index digest `sha256:f49565f188ee00bc2a18dd418183f2c5f23ef7d6e691890517ed341a598f67c3`. CI loads pull-request orchestration from the trusted base revision, checks out the exact PR head separately, and permits proposal validators to execute only when their executable authority closure is byte-identical to the trusted controller. Controller-changing proposals remain data-only until the resulting `main` commit validates itself. The workflow mounts the container tool root read-only, verifies the reviewed hosted-runner identity, selects a whole-file-digest-bound Linux lock, runs every accepted route under a poisoned caller `PATH`, and executes generated host binaries through `/proc/self/fd`. Each container is limited to 2 CPUs, 2 GiB of memory with no additional swap, and 256 processes; the aggregate job is limited to 30 minutes.
