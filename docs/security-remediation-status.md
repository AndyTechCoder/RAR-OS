...
| Role inventory cannot prove whole-rootfs absence | Implementation in progress | The non-activating `tools/rar-lab/rootfs-proof/` foundation resolves bounded OCI layout/index/manifest/config documents, verifies SHA-256 descriptors and uncompressed diff IDs, applies ordered ustar layers with OCI whiteout semantics, hashes every effective regular file, and finds executable/ELF objects across `/`. A trusted filesystem adapter, nested indexes, compression, evidence binding, and independent review remain required; keep v2 inactive |
