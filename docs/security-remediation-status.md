...
| Role inventory cannot prove whole-rootfs absence | Deferred implementation | Add a reviewed effective-rootfs proof/validator that resolves all OCI layers and enumerates every executable/loadable object across `/`; preserve the three-role topology, and keep the current v2 profile inactive until that proof is implemented |
