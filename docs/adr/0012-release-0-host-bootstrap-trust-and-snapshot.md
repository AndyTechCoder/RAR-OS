...
The versioned Class B inventory records version/identity, integrity pin, license, provenance, setup source, and status for every selected host-tool group, SDK, OCI image, GitHub action, and CI orchestration boundary. The hosted runner and container engine remain external attested service layers rather than part of the OCI digest. One non-container job checks the reviewed runner-image identity, exports the exact values through its own job environment, and launches the pinned read-only validation container on that same runner; the container/bootstrap independently reject missing or mismatched values. Cross-job runner-identity handoff is forbidden. Both service layers remain non-certifying: they may run current host tests but cannot support target certification or target-artifact reproducibility evidence.
