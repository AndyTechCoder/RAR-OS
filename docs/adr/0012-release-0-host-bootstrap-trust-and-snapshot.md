...
The versioned Class B inventory records version/identity, integrity pin, license, provenance, setup source, and status for every selected host-tool group, SDK, OCI image, GitHub action, and CI orchestration boundary. The hosted runner and container engine remain external attested service layers rather than part of the OCI digest. A non-container job checks the reviewed runner-image identity before the read-only container starts, exports the exact values through named job outputs, and the container/bootstrap independently reject missing or mismatched handoff values. Direct inheritance of runner-only environment variables into a container job is not assumed. Both service layers remain non-certifying: they may run current host tests but cannot support target certification or target-artifact reproducibility evidence.
