...
The architecture-adapter conformance tuple is expected architecture, external entry address and length, address width, page size, entry alignment, and stack alignment. The source provider is keyed by descriptor identity `(purpose,owner_kind,owner_id)`, not descriptor position. It records every copy and can produce bounded short reads and faults. A separate effect sink remains empty on rejection and commits entropy clearing, trace transfer, and device authority only after acceptance.
