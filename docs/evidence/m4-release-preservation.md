...
## Stable asset identity, not temporary draft URLs

The canonical record deliberately omits `browser_download_url`: draft URLs use temporary `untagged-...` paths and change when published (also documented in the upstream [release action implementation](https://github.com/softprops/action-gh-release/blob/master/src/run.ts)). The helper never follows or uses that field. Fixed repository/release API authority and exact positive asset ID, name, size, digest and uploaded state remain mandatory. The published release page supplies download links; stable IDs and SHA-256 bind the permanent evidence regardless of draft URL rewriting. Pure tests prove missing, temporary or arbitrary URL fields cannot enter the record or change its asset identity.
