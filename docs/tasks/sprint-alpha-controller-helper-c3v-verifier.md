...
Arbitrary bytes use the RFC 4648 standard Base64 alphabet with required canonical padding, no whitespace, and no alternate spelling. The retained decoded payload maximum is exactly 296816640 bytes. Because every chunk is padded independently, the Base64 payload total is `sum_i(4*ceil(chunk_i_decoded_bytes/3))` and its exact maximum is 396031116 bytes, not `4*ceil(total_decoded_bytes/3)`. Each chunk carries at most 1436 decoded bytes and one Base64 payload, so its complete framing record is at most 2048 bytes. At most 206697 maximally filled chunks are needed for the decoded total; the evidence retains the conservative absolute limit of 206741 chunks.
