...
Arbitrary bytes use the RFC 4648 standard Base64 alphabet with required canonical padding, no whitespace, and no alternate spelling. The retained decoded payload maximum is exactly 296816640 bytes. Because every chunk is padded independently, the Base64 payload total is `sum_i(4*ceil(chunk_i_decoded_bytes/3))` and the exact accepted payload cap is 396031116 bytes, not `4*ceil(total_decoded_bytes/3)`. Each chunk carries at most 1436 decoded bytes and one Base64 payload, so its complete framing record is at most 2048 bytes. At most 206697 maximally filled chunks are needed for the decoded total; the evidence retains the conservative absolute limit of 206741 chunks.
...
Every decoded blob is one canonical ASCII/LF envelope. It begins with the exact lines `rar-c3v-envelope-v0`, `kind=<kind>`, `case_id=<case-id>`, and `field_count=<unsigned-canonical-decimal>`, in that order. It then contains exactly `field_count` fields in the order required below. Field ordinal `NN` is two canonical decimal digits starting at `01`; each field is exactly four lines in this order: `field.NN.name=<name>`, `field.NN.bytes=<unsigned-canonical-decimal>`, `field.NN.sha256=<64-lowercase-hex>`, and `field.NN.base64=<canonical-base64>`. The final line has LF termination. Decimal zero is written only as `0`; other decimals have no leading zero. A zero-length field is exactly `bytes=0`, the SHA-256 of the empty string `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, and an empty value after `base64=`. No unlisted line, field, kind, case identity, whitespace, CR, or alternate spelling is valid.
