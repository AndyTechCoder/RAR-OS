...
Status: two-boundary redesign; M1.5 legacy cutover complete; M2 incomplete
...
The M1.5 workflow reaches only `tools/toolchain/preauth-transaction --prepare`. Until M2 is accepted, that command exits with the stable `preauth-transaction-m2-incomplete` refusal and emits no bundle, graph, artifact, disk, authority, or session.
