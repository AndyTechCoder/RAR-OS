...
Compatible higher minor versions may add only bounded unknown non-critical records or optional roles; decoders range-check and skip them. Unknown critical additions fail. Changed identity, required content, address-space meaning, required role, byte order, or bounds requires a new major version and parallel decoder. Platform discovery producers and consumers are replaceable when they produce equal owned values and pass the versioned corpus. Failure enters the R0 invalid-handoff recovery halt.
