...
Every record begins with `kind:u16`, `flags:u16`, `record_bytes:u32`, `record_id:u32`, `reserved:u32`. `record_id` is the sole identity and is unique within its kind; payloads never repeat it. References name a schema-fixed target kind. Flag bit 0 means critical; other bits and reserved fields are zero. Known v1.0 records use zero record flags. A compatible higher-minor register-window record may use bit 0 only to classify an otherwise unknown role as critical. Record size includes the prefix and is a nonzero multiple of 8. Records, including unknown and safely skipped records, are canonically ordered by `(kind, record_id)`. Unknown non-critical records are range-checked then skipped; unknown critical records fail.

## Required records

- CPU, 48 bytes: `flags:u32`, reserved `u32`, `hardware_id:u64`, `interrupt_controller_id:u32`, `timer_id:u32`, reserved 8 bytes. `hardware_id` is unique across all CPU records. Exactly one CPU has boot flag bit 0; its header ID equals handoff `boot_cpu_id`.
...
- Interrupt controller, 48 bytes: `model:u16`, `flags:u16`, `global_interrupt_base:u32`, `interrupt_count:u32`, reserved 20 bytes. The half-open global range is formed with checked addition, must not overlap another declared controller range, and must end at or below 256 for x86-64 or 1020 for AArch64. Models are x86 APIC (1) and ARM GICv3 (2).
...
- Register window, 56 bytes: `parent_kind:u16`, `role:u16`, `parent_id:u32`, `address_space:u16`, `access_width:u8`, `byte_order:u8`, `stride:u16`, `flags:u16`, `base:u64`, `length:u64`, `authority_purpose:u16`, and reserved fields. `parent_kind` must be interrupt (3), timer (4), or serial (5); the ID must resolve uniquely in that kind. `(authority_purpose,parent_kind,parent_id)` selects candidates, while address space, rights, and checked full-range containment select exactly one descriptor without using table position.

At least one CPU and memory record exists. Exactly one architecture-specific interrupt controller, timer, serial record, and boot source exists, and exactly one CPU carries the boot flag. Every CPU controller/timer reference and every timer/serial controller reference resolves uniquely. Dangling references, duplicate IDs within a kind, ambiguous targets, forbidden windows, missing required records, and extra singleton records fail.
...
Unknown roles in v1.0, model-incompatible roles or spaces, missing or extra required windows, overflow, and invalid width/stride return `invalid-register-window`. In a compatible higher minor, a register-window record with an unknown role and record critical bit clear is fully framed, included in identity and canonical-order checks, then skipped without resolving its parent, consuming a descriptor, or granting authority. The same unknown role with the critical bit set returns `unknown-critical`. Known roles may not set the record critical bit. I/O-port ranges must end at or below 65,536 and are valid only for x86-64.

Each window resolves exactly one Boot Entry device descriptor by matching `(purpose,parent_kind,parent_id)`, address space, read/write/device rights, authority transfer, and checked full-range containment. Descriptor reordering is immaterial. Multiple non-overlapping authority descriptors may belong to one controller, which is required for split GIC distributor and redistributor ranges; zero or multiple matching descriptors fail. Every declared device authority is consumed by exactly one matching window. System-memory windows must also lie within one device-owned MMIO memory-map entry; no window may cross an authority or map boundary, and windows belonging to different identities may not overlap. An RHD record or memory-map entry alone never grants access; a mismatch returns `unauthorized-device-window` before any register access.
...
Compatible higher minor versions may add only bounded unknown non-critical records or optional roles using the explicit record criticality encoding above; decoders range-check, identity-check, order-check, and then skip them without authority. Validators collect compatibility facts for the complete framed table, return `unknown-critical` if any critical unknown addition exists, otherwise return `unsupported-minor` if any noncritical addition lacks higher-minor authorization, and only then continue to identity and semantic validation. Physical record order never selects the compatibility result. Changed identity, required content, address-space meaning, required role, byte order, or bounds requires a new major version and parallel decoder. Platform discovery producers and consumers are replaceable when they produce equal owned values and pass the versioned corpus. Failure enters the R0 invalid-handoff recovery halt.
