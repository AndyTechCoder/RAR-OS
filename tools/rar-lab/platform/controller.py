"""Trusted-main Platform controller. Cloud-only; never execute on developer hosts."""
import base64
import json
import os
...
protocol = {"__name__": "trusted_platform_protocol"}
exec(compile((HERE / "protocol.py").read_text(), "trusted-platform-protocol.py", "exec"), protocol)
run, digest, sandbox = (helpers[name] for name in ("run", "digest", "sandbox"))
POLICY = helpers["POLICY"]

def kernel_state(tree):
    prefixes = ("nucleus/platform/", "core/platform/", "services/platform/")
    present = any(p.startswith(prefixes) or p in tuple(x[:-1] for x in prefixes) for p in tree)
    if not present:
        return "absent"
    required = ("nucleus/platform/main.rs", "nucleus/platform/model.rs",
                "core/platform/main.rs", "services/platform/model.rs")
...
def source_kind(base, source, changed):
    if "partial" in (base, source):
        raise ValueError("partial Platform source")
    if base == "complete" and source == "absent":
        raise ValueError("proposal removes Platform")
    if source == "complete":
        return "target"
    if base != "absent" or source != "absent":
        raise ValueError("invalid Platform source state")
    exact = {"AGENTS.md", "README.md", "SPRINT_STATUS.md",
             "tools/ci/check-sprint-static.sh", ".github/workflows/platform.yml"}
    if any(p not in exact and not p.startswith(("tools/rar-lab/platform/", "docs/"))
           for p in changed):
        raise ValueError("kernel-absent proposal exceeds controller/contract scope")
    return "controller-only"

def unpack(data):
    names = ("platform.efi", "platform.img", "platform-service.efi", "model-tests.log")
...
def self_test():
    ...
    assert kernel_state({"core/platform/main.rs": ("120000", "x")}) == "partial"
...
def main():
    ...
    evidence = workspace / "platform-evidence"
    ...
    summary = dict(source=os.environ["RAR_SOURCE_SHA"], controller=os.environ["RAR_CONTROLLER_SHA"],
                   ...
                   profile="q35-tcg-ps2-vga-private-qmp-v1")
    def save():
    ...
    try:
        ...
        if kind == "controller-only":
            summary.update(status=kind, target_execution="none: Platform implementation absent")
            save()
            print("Controller/contract validation only; no Platform implementation or boot claim.", flush=True)
            return
        for name in ("build", "launch"):
            tag = "rar-platform-" + name + ":" + run_id + "-" + attempt
        ...
        builds = []
        for index in (1, 2):
            name = "rar-platform-build-" + run_id + "-" + attempt + "-" + str(index)
            ...
            print("Independent Platform build " + str(index), flush=True)
        ...
        if summary["build_1"] != summary["build_2"]:
            raise ValueError("Platform clean builds differ")
        summary["reproducible"] = True
        directory = evidence / "boot"
        directory.mkdir()
        for name, raw in builds[0].items():
            if name != "model-tests.log":
                (directory / ("boot.img" if name == "platform.img" else name)).write_bytes(raw)
        name = "rar-platform-boot-" + run_id + "-" + attempt
        containers.append(name)
        ...
        summary.update(status="passed", records=records,
                       serial_sha256=digest(serial), frame_sha256=digest(frame),
                       qemu_exit=0, injected_keys=["a"])
        save()
        print("Platform: two reproducible builds, ring3 proofs, real input and pixels passed.", flush=True)
