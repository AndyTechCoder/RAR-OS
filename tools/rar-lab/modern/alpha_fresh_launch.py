"""Fixed cloud-only native Alpha fresh launch. No arguments, environment-selected paths or local mode."""
...
session.cloud_guard()
try:
    result=session.load("alpha_scenario").run(session,"fresh")
except Exception as error:
    result=dict(schema="rar-native-alpha-failure-v1",status="failed",mode="fresh",
