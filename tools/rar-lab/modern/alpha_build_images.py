"""Private four-image native Alpha framing; unchanged per-service PE limits."""
...
def unpack(data,inspect):
    ...
    lines=data.split(b"\n")
    if len(lines)!=10 or lines[-2:]!=[b"RAR-SIGNED-BUILD:END",b""]:
        raise ValueError("signed build transfer framing")
    result={}
    for i,name in enumerate(("modern.efi","modern-service.efi","modern-network.efi","modern-compositor.efi")):
    ...
    for name in ("modern-network.efi","modern-compositor.efi"):
        if result["modern.efi"].count(result[name])!=1:
            raise ValueError("exact fixed private service embedded once: "+name)
    return result
