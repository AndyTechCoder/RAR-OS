...
spec=importlib.util.spec_from_file_location("expansion_release_evidence",Path(__file__).with_name("expansion_release_evidence.py"))
...
class Tests(unittest.TestCase):
    ...
    def test_fixed_api_only(self):
        ...
        for kind in subject.KINDS:
            path=root+"/assets?name=m5-"+kind+"-"+SOURCE[:12]+"-proof.zip"
    ...
    def exercise(self,failure=None,resume=False,interrupt=None,publish_at=None,publisher_source=SOURCE):
        ...
        if failure=="failed-run":runs[204]["conclusion"]="failure"
        if failure=="wrong-source":runs[204]["head_sha"]="b"*40
        if failure=="wrong-branch":runs[204]["head_branch"]="codex/proposal"
        if failure=="legacy-push":runs[204]["event"]="push"
        if failure=="spec-dispatch":runs[999]["event"]="workflow_dispatch"
        if failure=="publisher-run":runs[204]["head_sha"]=publisher_source
        if failure=="publisher-title":runs[204]["display_title"]="Desktop "+publisher_source
        if failure=="publisher-artifact":metadata[104]["workflow_run"]["head_sha"]=publisher_source
        ...
        if failure=="wrong-artifact":metadata[104]["workflow_run"]["id"]=999
        if failure=="old-attempt":runs[204]["run_attempt"]=2
        if failure=="expired":metadata[104]["expired"]=True
    ...
    def test_malformed_asset_responses(self):
        name="m5-alpha-"+SOURCE[:12]+"-proof.zip";raw=b"proof"
