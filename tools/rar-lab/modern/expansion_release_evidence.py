"""Copy exact successful cloud evidence to one existing draft M5 release.
...
REPO="AndyTechCoder/RAR-OS";TAG="v0.5.0-expansion-alpha"
KINDS={
...
def selection(value):
    ...
    if total>256*1024**2:raise ValueError("total artifact budget")
...
def promote(github,api,release_id,source,plan,sha,canonical,publisher_source=None):
    ...
    for row in rows:
        ...
        name="m5-"+row["kind"]+"-"+source[:12]+"-proof.zip"
    ...
    record=dict(schema="rar-expansion-release-evidence-v0",source=source,publisher_source=publisher_source,tag=TAG,
...
def main():
    ...
    def api(method,path,raw=None):
        ...
        request=urllib.request.Request(url,data=raw,method=method,headers={
            ...
            "User-Agent":"RAR-M5-Release-Evidence"})
