...
[ "$(/usr/bin/printf '%s\n' "$ephemeral" | /usr/bin/awk 'NF { count++ } END { print count + 0 }')" -eq 27 ] || exit 1
...
[ "$(/usr/bin/grep -Fxc "printf '%s\\n' 'Ephemeral policy tests passed: executed=27 source=read-only scratch=tmpfs'" "$runner")" -eq 1 ] || exit 1
...
printf '%s\n' 'Ephemeral policy-test confinement passed: ephemeral=27 immutable=5 source=read-only'
