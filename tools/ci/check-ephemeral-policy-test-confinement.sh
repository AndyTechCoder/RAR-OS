...
[ "$(/usr/bin/printf '%s\n' "$ephemeral" | /usr/bin/awk 'NF { count++ } END { print count + 0 }')" -eq 28 ] || exit 1
...
[ "$(/usr/bin/grep -Fxc "printf '%s\\n' 'Ephemeral policy tests passed: executed=28 source=read-only scratch=tmpfs'" "$runner")" -eq 1 ] || exit 1
...
printf '%s\n' 'Ephemeral policy-test confinement passed: ephemeral=28 immutable=5 source=read-only'
