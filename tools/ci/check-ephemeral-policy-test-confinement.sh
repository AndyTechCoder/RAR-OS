...
[ "$(/usr/bin/printf '%s\n' "$ephemeral" | /usr/bin/awk 'NF { count++ } END { print count + 0 }')" -eq 25 ] || exit 1
...
printf '%s\n' 'Ephemeral policy-test confinement passed: ephemeral=25 immutable=5 source=read-only'
