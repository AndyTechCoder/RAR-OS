...
/usr/bin/awk -F '=' '
    ...
    END {
        ...
        if (value["test_count"] != "7" || value["failed_count"] != "0" || value["network"] != "none" || value["observed_exit_status"] != "0" || value["status"] != "accepted") reject("test result invalid")
        exit bad ? 1 : 0
    }
' "$evidence" || exit 1
/usr/bin/awk -F '|' '
    BEGIN { split("official-short-vectors streaming-boundaries-match-one-shot round-trip-and-layout matches-language-neutral-golden-vector accepts-every-phase-role-kind-combination rejects-noncanonical-values rejects-mutated-wire-rules", expected, " ") }
    NR == 1 { if ($0 != "schema=rar-alpha-controller-helper-test-cases-v0") bad=1; next }
    NR > 1 { if (NR > 8 || NF != 2 || $1 != expected[NR-1] || $2 != "pass" || ++seen[$1] != 1) bad=1; count++ }
    END { if (count != 7) bad=1; exit bad ? 1 : 0 }
...
printf '%s\n' 'controller helper test evidence context validated: cases=7 status=accepted'
