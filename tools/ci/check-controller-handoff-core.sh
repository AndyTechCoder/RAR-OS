...
for file in README.md build-plan.v0 lib.rs manifest.rs sha256.rs fixtures/manifest-golden.v0.hex; do
...
/usr/bin/grep -qx 'status=codec-source-only-no-copy-authority' "$plan" || fail 'authority state mismatch'
