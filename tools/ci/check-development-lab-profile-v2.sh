...
/usr/bin/awk -F '=' '
    ...
    END {
        if (lines != 38) reject("field count is invalid")
        ...
        if (value["acceptance_plan_path"] != "spec/alpha/evidence/acceptance-v2.plan" || value["acceptance_plan_sha256"] != "ffdb07b584abc94122b14a416593916cf18df439de042c97ff83fda9e4444ccd") reject("acceptance protocol identity is invalid")
...
[ -f "$plan" ] && [ ! -L "$plan" ] || fail 'acceptance v2 plan is unavailable'
if [ -x /usr/bin/sha256sum ]; then
    plan_sha256=$(/usr/bin/sha256sum "$plan" | /usr/bin/awk '{ print $1 }')
else
    plan_sha256=$(/usr/bin/shasum -a 256 "$plan" | /usr/bin/awk '{ print $1 }')
fi
[ "$plan_sha256" = ffdb07b584abc94122b14a416593916cf18df439de042c97ff83fda9e4444ccd ] || fail 'acceptance v2 plan digest mismatch'
