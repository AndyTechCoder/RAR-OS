...
if /usr/bin/grep -n '^
for name in RAR_EXPECTED_REPOSITORY RAR_EXPECTED_REF RAR_EXPECTED_EVENT     RAR_TRUSTED_CONTROLLER_SHA RAR_EXPECTED_SOURCE_REVISION RAR_EXPECTED_RUN_ID     RAR_EXPECTED_RUN_ATTEMPT RAR_CI_RUNNER_IMAGE_OS RAR_CI_RUNNER_IMAGE_VERSION     RAR_CI_RUNNER_OS RAR_CI_RUNNER_ARCH RAR_CI_BOOTSTRAP_IMAGE     RAR_EXPECTED_WRAPPER_SHA256 RAR_EXPECTED_SUBJECT_SHA256     RAR_EXPECTED_FIXTURE_SHA256 RAR_EXPECTED_TOOL_PINS_SHA256     RAR_EXPECTED_ARTIFACT_NAME RAR_EXPECTED_RECORD_NONCE; do
    eval "value=\${$name-}"
...
for value in "$RAR_EXPECTED_WRAPPER_SHA256" "$RAR_EXPECTED_SUBJECT_SHA256"     "$RAR_EXPECTED_FIXTURE_SHA256" "$RAR_EXPECTED_TOOL_PINS_SHA256" "$RAR_EXPECTED_RECORD_NONCE"; do
...
if /usr/bin/grep -n '^expected="schema=rar-alpha-controller-helper-closure-observer-run-evidence-v0
...
printf '%s\n' 'controller-helper observer run evidence validated: exact-main candidate-not-reviewed-not-ready'
 "$record" >/dev/null; then fail 'record contains blank line'; fi
...
[ "$(/usr/bin/sed -n '31p' "$record")" = "record_sha256=$record_sha" ] || fail 'record digest mismatch'
printf '%s\n' 'controller-helper observer run evidence validated: exact-main candidate-not-reviewed-not-ready'
 "$case_evidence" >/dev/null; then fail 'case evidence contains blank line'; fi
[ "$(/usr/bin/wc -l < "$case_evidence" | /usr/bin/tr -d ' ')" -eq 23 ] || fail 'case evidence line count invalid'
/usr/bin/awk -F '|' -v controller="$RAR_TRUSTED_CONTROLLER_SHA" -v source="$RAR_EXPECTED_SOURCE_REVISION" -v subject="$RAR_EXPECTED_SUBJECT_SHA256" -v fixture="$RAR_EXPECTED_FIXTURE_SHA256" -v pins="$RAR_EXPECTED_TOOL_PINS_SHA256" '
...
NR>2 {
  ordinal=NR-2; id=sprintf("O%03d",ordinal)
