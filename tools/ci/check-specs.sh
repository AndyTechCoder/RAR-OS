...
[ "$(awk -F '|' 'NR > 2 && NF { count++ } END { print count + 0 }' spec/fixtures/release-0/conformance-scenarios.v1)" -eq 174 ] || fail "R0-002 architecture/provider conformance scenarios are incomplete"
printf '%s\n' \
