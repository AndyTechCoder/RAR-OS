...
[ "$c2_packet_sha256" = e7ba535591d292e508dbffb18f83cf7ceee58916799e46b3cd5bbc09bcdb2305 ] || fail "ADR 0024 C2 packet changed without review"
...
grep -Fqx 'It first copies exactly `/source/.git` to `/destination/.git`, then copies' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits metadata-first transfer"
grep -Fqx 'the complete `/source/.` tree to `/destination/`, and only afterward applies' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits complete transfer"
