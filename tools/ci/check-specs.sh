...
[ "$c3v_packet_sha256" = bfdb2dfb0b49bc34ffe5fbfcb7d5fc53499e179e834c161885f2d4f2bd559267 ] || fail "ADR 0024 C3V packet changed without review"
...
grep -Fqx 'C3V is split into two separately reviewed and merged subphases:' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V packet lost its two-merge boundary"
grep -Fqx 'The existing verifier runtime and all C2 production paths remain read-only during C3VA.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA packet broadened runtime authority"
grep -Fqx 'The existing `tools/ci/verify-controller-helper-closure-candidate.sh` is read-only in C3VB. Every semantic contract or path not named above remains read-only.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB packet broadened verifier authority"
