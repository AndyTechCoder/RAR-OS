...
[ "$c3v_packet_sha256" = 0a7e666e959e18671e69414b43dc1dbf6835d62d3d355ed8c495a5269b1e7de4 ] || fail "ADR 0024 C3V packet changed without review"
...
grep -Fq 'Because every chunk is padded independently, the Base64 payload total is `sum_i(4*ceil(chunk_i_decoded_bytes/3))` and the exact accepted payload cap is 396031116 bytes, not `4*ceil(total_decoded_bytes/3)`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost independent-padding bound"
