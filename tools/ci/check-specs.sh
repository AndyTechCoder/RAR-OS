...
[ "$c3v_packet_sha256" = 9520fe02d9731ca06388f4150238e9e46ab2b4e0ecaf83a6af760d9ecc87192e ] || fail "ADR 0024 C3V packet changed without review"
...
grep -Fqx 'The retained decoded payload maximum is exactly 296816640 bytes; its Base64 payload maximum is exactly `4*ceil(decoded_bytes/3) = 395755520` bytes. Each chunk carries at most 1436 decoded bytes and one Base64 payload, so its complete framing record is at most 2048 bytes. The evidence contains at most 206741 chunks.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost decoded/chunk bounds"
...
grep -Fqx 'The evidence file maximum is 440401920 bytes. Its conservative proof is `395755520 + 206741*128 + 709*256 + 209*2048 + 8192 = 422836096`. The exact six-file artifact maximum is 441473024 bytes, below the existing 536870912-byte output budget. Raw stdout plus stderr remains at most 65536 decoded bytes per runtime case.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost evidence bound"
grep -Fqx 'C3VA is incomplete while any named repeatability, retention, reconstruction, nonaliasing, or normalization gap remains.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA permits unresolved evidence gaps"
