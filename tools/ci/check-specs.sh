...
[ "$c3v_packet_sha256" = 76713e7d37aa7b1217043041bb446d7b00f4eec6f9f7c28f918be4c613f83d9c ] || fail "ADR 0024 C3V packet changed without review"
...
grep -Fq 'Because every chunk is padded independently, the Base64 payload total is `sum_i(4*ceil(chunk_i_decoded_bytes/3))` and its exact maximum is 396031116 bytes, not `4*ceil(total_decoded_bytes/3)`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost independent-padding bound"
...
grep -Fq 'Every decoded blob is one canonical ASCII/LF envelope.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost decoded envelope grammar"
grep -Fq 'A zero-length field is exactly `bytes=0`, the SHA-256 of the empty string `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, and an empty value after `base64=`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost canonical zero representation"
...
grep -Fqx 'The evidence file maximum is 440401920 bytes. Its conservative proof is `396031116 + 206741*128 + 709*256 + 209*2048 + 8192 = 423111692`. The exact six-file artifact maximum is 441473024 bytes, below the existing 536870912-byte output budget. Raw stdout plus stderr remains at most 65536 decoded bytes per runtime case.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost evidence bound"
