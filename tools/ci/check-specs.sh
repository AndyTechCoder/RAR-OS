...
[ "$c3v_packet_sha256" = b003f286766fc8227e46155e924c192857ae16d92fb66c88f69fd1f7d73a38aa ] || fail "ADR 0024 C3V packet changed without review"
...
grep -Fqx 'C3VA adds exactly `tools/ci/test-controller-helper-closure-verifier-evidence-policy.sh|ephemeral` to `tools/ci/policy-test-modes.v0`, changing the registry from 28 to 29 entries. The runner invokes all 29 exactly once in registry order and reports exactly `policy-tests=29`. The confinement checker recognizes exactly that new path and mode. No other registry row, order, mode, runner behavior, or confinement exception changes.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost exact policy registry transition"
grep -Fqx 'The evidence maximum is 440401920 bytes. Its conservative proof is `395755520 + 206741*128 + 709*256 + 209*2048 + 8192 = 422836096`. The exact six-file artifact maximum is 441473024 bytes, below the existing 536870912-byte output budget. Raw stdout plus stderr remains at most 65536 bytes per runtime case. A header is at most 8192 bytes and each normalized case/proof record is at most 2048 bytes.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost evidence bound"
...
grep -Fqx 'C3VB adds exactly `tools/ci/test-controller-helper-closure-verifier-policy.sh|ephemeral` to the 29-entry registry, producing exactly 30 entries. The runner invokes all 30 once in registry order and reports exactly `policy-tests=30`. The confinement checker recognizes only that one added path/mode. No existing row, mode, ordering, or C3VA test changes.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost exact policy registry transition"
grep -Fqx 'Its only automatic trigger is `push` to `main` with the literal path filter `spec/alpha/lab/controller-helper-closure-verifier-activation-v0.fields`. The activation record is created once in C3VB and is immutable afterward. The job proves:' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost one-shot trigger"
grep -Fqx 'C3VB invokes the frozen observer harness directly in its own fresh isolated container to produce exactly the fresh manifest and 23-line observation receipt for the exact C3VB merge revision. It does not invoke the C2 wrapper, consume C2 outer run evidence, or reuse any prior artifact. The controller independently validates and freezes those two files before verification.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost fresh direct observation"
