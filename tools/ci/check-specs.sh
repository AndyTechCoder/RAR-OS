...
grep -q 'ubuntu-24.04-runtime-attested' "$class_b_inventory" || fail "Class B inventory omits the runtime-attested runner boundary"
...
grep -Fq 'rar_ci_image_version=${ImageVersion-}' .github/workflows/specifications.yml || fail "CI runner image version is not recorded and validated"
