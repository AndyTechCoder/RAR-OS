...
[ -x tools/ci/verify-controller-helper-closure-verifier-evidence.sh ] || fail "C3VA evidence validator is not executable"
[ -x tools/ci/test-controller-helper-closure-verifier-evidence-policy.sh ] || fail "C3VA evidence policy test is not executable"
[ "$(grep -Fxc 'tools/ci/test-controller-helper-closure-verifier-evidence-policy.sh|ephemeral' tools/ci/policy-test-modes.v0)" -eq 1 ] || fail "C3VA evidence policy registry row missing or duplicated"
...
[ "$c2_packet_sha256" = 545157f19b0c21b758742bfa08f29d8b3603b28813de7c6437003f98458d0246 ] || fail "ADR 0024 C2 packet changed without review"
...
grep -Fqx -- '- `tools/toolchain/host-tools.manifest`' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits dependent host-tool manifest ownership"
grep -Fqx -- '- `tests/bootstrap/src/main.rs`' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits exact-main inventory assertion repair ownership"
grep -Fqx 'Post-merge exact-main remediation authority for `tests/bootstrap/src/main.rs` is limited to synchronizing its expected Class B inventory ID set with the already-reviewed `tools/toolchain/class-b-host-tools.v1`. It may not change route behavior, production code, tool identities, compiler/helper/target authority, readiness, or any other assertion.' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet broadens exact-main inventory assertion repair authority"
...
grep -Fqx 'The keeper runs as fixed UID/GID `65532:65532`, distinct from the attested' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits distinct keeper identity"
grep -Fqx '`/usr/bin/dash` command must prove that `/keep` is neither readable nor' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits keeper access denial"
grep -Fqx 'mount but receives no readable source authority. The existing immutable' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet grants readable keeper source authority"
grep -Fqx '`timeout-minutes: 15` observe-job deadline bounds this wait and all cleanup;' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits bounded keeper lifetime"
grep -Fqx 'before acquisition, after acquisition removal, and immediately before transfer.' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits keeper lifecycle checks"
grep -Fqx 'It copies the complete `/source/.` tree exactly once to `/destination/`; this' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits single complete transfer"
grep -Fqx 'single archive-preserving copy includes `.git` and prevents a second pass from' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits metadata inclusion"
grep -Fqx 'overwriting immutable Git pack files. Only afterward it applies the' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet permits duplicate immutable-pack overwrite"
grep -Fqx 'Normal cleanup order is transfer, keeper, then volume after acquisition has' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits normal cleanup order"
grep -Fqx 'already been removed. Failure cleanup order is acquisition, transfer, keeper,' docs/tasks/sprint-alpha-controller-helper-c2-observer.md || fail "C2B packet omits failure cleanup order"
...
c3v_packet_sha256=$(sha256_of docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md) || fail "cannot hash ADR 0024 C3V packet"
[ "$c3v_packet_sha256" = 99ba1ad46dccc75399711e379d3897ae7950e79333749ba975bdb82e3cd8c1bd ] || fail "ADR 0024 C3V packet changed without review"
grep -Fqx 'Status: authoritative source-only child packet; implementation is phase-gated.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V packet status changed"
grep -Fqx 'D0 only registers and byte-binds this packet. D0 grants no C3VA, C3VB, C3VR, workflow, container, verifier, compiler, helper, target, acceptance, or readiness authority. D0 must receive exact-head architecture, correctness, and security review, merge unchanged, and pass a distinct exact-main Specifications run including the complete mutation suite before C3VA starts.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V packet lost registration gate"
...
grep -Fqx '1. **C3VA - contract and evidence activation.** Close the repeatability/evidence gap and activate only validators and static policy tests. C3VA adds or activates no workflow, container, or verifier runtime and never executes the existing dormant verifier source.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA incorrectly activates runtime"
...
grep -Fq 'At most 206697 maximally filled chunks are needed for the decoded total; the evidence retains the conservative absolute limit of 206741 chunks.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost decoded/chunk bounds"
grep -Fqx 'There are exactly 709 typed blobs: four per runtime case (166*4=664), one per residual proof (43), and two run-global clean-success pass blobs, for `664+43+2=709`. Each runtime case has one pre/input/topology envelope, one stdout blob, one stderr blob, and one post/output/event/resource envelope. Each residual proof has one lossless source/proof envelope. The two run-global blobs are respectively the clean-success pass-1 and pass-2 lossless envelopes; between them they bind the domain/header/tool/fixture inputs and each carries its pass topology, canonical manifest, mount identities, output, event, and resource bytes. The validator independently recomputes pass equality and every verification-receipt binding from those two retained preimages. No blob exceeds 16777216 decoded bytes; the total decoded cap still governs. Every blob has one header of at most 256 bytes and exact type, case/domain identity, decoded length, chunk count, and SHA-256. The evidence has one header of at most 8192 bytes, exactly 209 normalized records of at most 2048 bytes, and at most 207660 logical records total: one header, 709 blob headers, 206741 chunk records, and 209 normalized records.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost clean-success/blob/line derivation"
grep -Fq 'Every decoded blob is one canonical length-framed envelope: ASCII/LF framing surrounds arbitrary raw field bytes, and the envelope itself is encoded only once by the outer canonical Base64 chunk records.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost decoded envelope grammar"
grep -Fq 'A zero-length field is exactly `bytes=0`, the SHA-256 of the empty string `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, `field.NN.data` plus LF, zero data bytes, and the terminating LF.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost canonical zero representation"
grep -Fq 'For every field, the validator must consume exactly the declared byte count, require SHA-256 of those exact raw bytes to equal the declared digest, and use checked arithmetic to prove that the actual field count and the sum of all framing and field bytes exactly equal the containing blob' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost decoded field equality and checked aggregation"
grep -Fq 'The blob SHA-256 is over the complete decoded envelope, including framing, raw field bytes, and terminating LFs.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost complete envelope hash domain"
grep -Fqx 'The only kinds and their exact ordered field names are:' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost exact envelope kinds"
grep -Fq 'Blob identities are exactly `B000001..B000709`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost deterministic blob allocation"
grep -Fq '`raw_blob_ids` is a nonempty comma-separated list of canonical blob identities with no spaces.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost raw blob identity grammar"
grep -Fq 'The normalized digest is SHA-256 over those exact ASCII/LF bytes.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost normalized digest preimage"
grep -Fq 'The evidence header has one exact canonical value for `repository`, `controller_sha`, `source_sha`, `run_id`, `run_attempt`, `verification_receipt_sha256`, `artifact_nonce`, `blob_count`, `chunk_count`, `decoded_bytes`, and `normalized_count`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost trusted header tuple"
grep -Fq 'Header `blob_count` equals the 709 actual blob headers, `chunk_count` equals both the actual chunk-record count and the checked sum of all blob-declared chunk counts' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost aggregate equality"
grep -Fq 'It does not claim global historical nonce uniqueness.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA overclaims stateless replay prevention"
grep -Fqx 'The evidence file maximum is 440401920 bytes. Its conservative proof is `396032120 + 206741*128 + 709*256 + 209*2048 + 8192 = 423112696`. The exact six-file artifact maximum is 441473024 bytes, below the existing 536870912-byte output budget. Raw stdout plus stderr remains at most 65536 decoded bytes per runtime case.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA lost evidence bound"
grep -Fq 'C3VA is incomplete while any named repeatability, retention, reconstruction, nonaliasing, or normalization gap remains.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VA permits unresolved evidence gaps"
grep -Fqx 'Every C3VA semantic contract, source checker, evidence validator, evidence policy test, and evidence fixture is frozen in C3VB by its exact C3VA-main SHA-256 digest. C3VB may consume but must not modify those files. The existing `tools/ci/verify-controller-helper-closure-candidate.sh`, direct production observer `tools/ci/observe-controller-helper-closure.sh`, and `spec/alpha/lab/controller-helper-closure-observation-v0.fields` are also frozen by exact reviewed digest and remain read-only.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB can rewrite its oracle"
grep -Fqx 'C3VB adds exactly `tools/ci/test-controller-helper-closure-verifier-policy.sh|ephemeral` to the 34-row registry. Ephemeral rows change 29 to 30; immutable rows remain exactly 5; total test rows change 34 to 35. The runner invokes all 30 ephemeral tests exactly once in registry order and reports exactly `Ephemeral policy tests passed: executed=30 source=read-only scratch=tmpfs`. The confinement checker recognizes only that one added path and mode. No existing row, mode, ordering, summary shape, or C3VA test changes.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost exact policy registry transition"
grep -Fq 'Before any repository code runs, the job proves canonical repository, push event, main ref, Linux/X64 runner, `github.event.after == github.event.head_commit.id == GITHUB_SHA`, `github.event.before` equals the literal exact C3VA-main SHA frozen during C3VB, and `github.run_attempt == 1`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost pre-code one-shot identity"
grep -Fq 'The external merge gate constructs the merge with only the exact C3VB PR head that passed all three reviews and checks as parent 2. Independent post-run evidence review must compare the recorded parent 2 to that established reviewed head before C3VB completes; the workflow proves topology and byte identity while the external gate proves review authority without a circular self-hash.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost reviewed parent-2 binding"
grep -Fq 'Exactly two isolated, bounded pre-verification network phases are permitted: anonymous provisioning of that exact platform/image digest only when inventory proves it absent, and anonymous exact-revision depth-bounded source acquisition. Each phase has a distinct container identity, empty mode-0700 Docker configuration, lifetime, output bound, and cleanup/absence proof. Both containers and their configurations are terminated and removed before observation or verification. No other network is allowed; every subsequent container uses `--network none` and `--pull=never`.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost exact network state machine"
grep -Fqx 'C3VB invokes the frozen production observer `tools/ci/observe-controller-helper-closure.sh` directly in its own fresh isolated container to produce exactly the fresh manifest and 23-line observation receipt for the exact C3VB merge revision. It does not invoke the C2 wrapper, the O001-O021 observer harness, C2 outer validation, or any prior artifact. The controller independently validates and freezes those two files against the frozen observation contract before verification.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost fresh direct observation"
grep -Fqx '`tools/ci/run-controller-helper-closure-verifier.sh` is the sole host-side Docker controller. No other changed file may invoke Docker or access `unix:///var/run/docker.sock`. The socket is never mounted into a container. Ambient `DOCKER_HOST`, `DOCKER_CONTEXT`, `DOCKER_TLS_VERIFY`, `DOCKER_CERT_PATH`, and `DOCKER_CONFIG` must be absent. The controller uses a fresh empty mode-0700 Docker config and the exact `unix:///var/run/docker.sock` endpoint.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost sole controller boundary"
grep -Fqx 'The final artifact contains exactly these six files:' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost exact retained set"
grep -Fqx 'The artifact name is exactly `controller-helper-closure-verification-${{ github.run_id }}-${{ github.run_attempt }}`. A single final upload uses `actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02`, `retention-days: 14`, `overwrite: false`, and `include-hidden-files: false`. It occurs only after producer termination, cleanup/absence proof, and an independent validator has reconstructed every normalized result from the retained raw preimages and byte-validated the exact six-file set. Nothing else is retained.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3VB lost exact upload boundary"
grep -Fqx 'C3VR does not delete any file. It replaces the workflow with a byte-pinned inert archival workflow having no automatic trigger and one manual-only job whose job-level condition is literal false, so no runner or repository code can execute. Its source/policy checks reject any active automatic trigger, true job condition, verifier/controller invocation, artifact upload, credential, permission expansion, or network action. The registry remains exactly 35 test rows: 30 ephemeral and 5 immutable. Every C3VA/C3VB evidence/runtime byte remains unchanged.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V lost mandatory safe retirement"
grep -Fqx 'C3V must not contain or change a closure-acceptance instance, controller/helper source, target source, observer production bytes, locks, inventories, approved profiles, release gates, compiler policy, boot policy, or readiness state except for the literal status-document paths explicitly named above. It must not compile or execute any helper, target, firmware, guest, kernel, bootloader, QEMU workload, or RAR OS artifact. It grants no compiler-use, target-execution, acceptance, signing, release, or readiness authority.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V packet lost denied authority"
grep -Fqx 'All mutations are repository-confined and GitHub-hosted. RAR OS target code must not be built, run, booted, mounted, or executed on the Mac or SSD. No local or SSD file may be created, changed, moved, or deleted. The SSD may be used only as separately authorized source/worktree storage; this packet grants no such authorization.' docs/tasks/sprint-alpha-controller-helper-c3v-verifier.md || fail "ADR 0024 C3V packet lost host/storage safety"
grep -Fqx -- '- [Sprint Alpha ADR 0024 C3V exact-set verification packet](tasks/sprint-alpha-controller-helper-c3v-verifier.md)' docs/README.md || fail "ADR 0024 C3V packet is not indexed"
