...
find "$tree" -name '._*' -print | /usr/bin/grep -q . && exit 1
find "$tree" -type l -print | /usr/bin/grep -q . && exit 1
count=$(find "$tree" -type f -print | /usr/bin/wc -l | /usr/bin/tr -d ' ')
...
find "$tree" -type f -print | /usr/bin/sed "s|^$tree/||" | /usr/bin/sort > "$paths"
: > "$manifest"
