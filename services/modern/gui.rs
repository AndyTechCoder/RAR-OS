...
#[cfg(test)] mod tests {
    ...
    #[test] fn high_incarnations_do_not_alias_legacy_low_bits(){
        ...
        let mut c=Compositor::new(peers).unwrap();
        assert!(c.apply(0,3,&Windows::new().wire()).is_err());
        assert_eq!(c.apply(0,peers[0],&Windows::new().wire()),Ok(true));
        assert!(c.apply(5,u32::MAX as u64,&begin(1)).is_err());
        assert_eq!(c.apply(5,peers[5],&begin(1)),Ok(false));
        for role in [0usize,4,5,6]{let mut bad=peers;bad[role]=0;assert!(Compositor::new(bad).is_err());}
        assert!(c.apply(u64::MAX,1,&begin(1)).is_err());
    }
    #[test] fn surfaces_commit_atomically_by_kernel_sender() {
        let mut c=Compositor::new([1;10]).unwrap();put(&mut c,4,1,b"FILES");put(&mut c,6,1,b"TERM");
    ...
    #[test] fn malformed_and_duplicate_surface_lines_preserve_commit() {
        let mut c=Compositor::new([1;10]).unwrap();put(&mut c,4,1,b"OLD");c.apply(4,1,&begin(2)).unwrap();
    ...
    #[test] fn replacement_begin_only_compares_committed_version() {
        let mut c=Compositor::new([1;10]).unwrap();put(&mut c,4,1,b"OLD");
